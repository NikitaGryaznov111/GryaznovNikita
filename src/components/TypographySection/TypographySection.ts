/**
 * Рендерит секцию с примерами типографики и кнопками переключения между десктопной и мобильной версиями.
 *
 * Функция:
 * 1. Находит элемент с id "grid" в DOM.
 * 2. Если элемент найден, вставляет после него HTML-разметку с таблицей стилей типографики.
 * 3. Добавляет обработчики событий на кнопки "Десктоп" и "Телефон", чтобы переключать отображение соответствующих колонок с типографикой.
 *
